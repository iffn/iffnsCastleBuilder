...
public class TriangleMeshInfo
{
    ...
    public void GenerateUVMeshBasedOnCardinalDirections(Transform meshObject, Transform originObjectForUV)
    {
        ...
        Vector3 normal = Quaternion.Inverse(meshObject.rotation) * originObjectForUV.rotation * currentTriangle.NormalVector;
        ...
        switch (direction)
        {
            case Direction.yz0:
                ...
                foreach (Vector3 baseVector in AllVerticesDirectly)
                {
                    Vector3 originVector = MathHelper.ConvertPointIntoOriginTransform(baseObject: meshObject, originObject: originObjectForUV, vector: baseVector);

                    //UVs.Add(new Vector2(originVector.z + localOffset.z, originVector.y + localOffset.y));
                    UVs.Add(new Vector2(originVector.z, originVector.y));
                }
                break;
            case Direction.xz0:
                UVs.Clear();
                ...
                foreach (Vector3 baseVector in AllVerticesDirectly)
                {
                    ...
                    //UVs.Add(new Vector2(originVector.x + localOffset.x, originVector.z + localOffset.z));
                    UVs.Add(new Vector2(originVector.x, originVector.z));
                }
                break;
            case Direction.xy0:
                UVs.Clear();
                ...
                foreach (Vector3 baseVector in AllVerticesDirectly)
                {
                    ...
                    //UVs.Add(new Vector2(originVector.x + localOffset.x, originVector.y + localOffset.y));
                    UVs.Add(new Vector2(originVector.x, originVector.y));
                }
                break;
            case Direction.Tilted:
                //ToDo: Not normal, ignore
        ...
        /*
        for (int i = 0; i<triangleInfo.Triangles.Count; i++)
        {
            TriangleHolder currentTriangle = triangleInfo.Triangles[i];

            Vector3 normal = currentTriangle.NormalVector;

            Direction direction;

            if (MathHelper.FloatIsZero(normal.x))
            {
                direction = Direction.x0;
            }
            else if (MathHelper.FloatIsZero(normal.y))
            {
                direction = Direction.y0;
            }
            else if (MathHelper.FloatIsZero(normal.z))
            {
                direction = Direction.z0;
            }
            else
            {
                direction = Direction.Tilted;
            ...
            {
                case Direction.x0:
                    UVs.Add(new Vector2(vertices[0].z, vertices[0].y));
                    UVs.Add(new Vector2(vertices[1].z, vertices[1].y));
                    UVs.Add(new Vector2(vertices[2].z, vertices[2].y));
                    break;
                case Direction.y0:
                    UVs.Add(new Vector2(vertices[0].x, vertices[0].z));
                    UVs.Add(new Vector2(vertices[1].x, vertices[1].z));
                    UVs.Add(new Vector2(vertices[2].x, vertices[2].z));
                    break;
                case Direction.z0:
                    UVs.Add(new Vector2(vertices[0].x, vertices[0].y));
                    UVs.Add(new Vector2(vertices[1].x, vertices[1].y));
                    UVs.Add(new Vector2(vertices[2].x, vertices[2].y));
                    break;
                case Direction.Tilted:
                    //ToDo: Not normal
                    UVs.Add(new Vector2(vertices[0].z, vertices[0].y));
